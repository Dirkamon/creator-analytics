...
describe("database TLS options", () => {
  it("uses the Supabase root CA for verified hosted connections", () => {
    const options = getDatabaseSslOptions(
      "postgresql://creator_analytics_web_reader.project-ref:placeholder@aws-0-us-west-2.pooler.supabase.com:5432/postgres?sslmode=verify-full",
    );
  ...
  it("keeps local validation connections unencrypted", () => {
    expect(
      getDatabaseSslOptions(
        "postgresql://creator_analytics_web_reader:placeholder@127.0.0.1:5432/postgres?sslmode=disable",
      ),
    ).toBe(false);
  });
});
