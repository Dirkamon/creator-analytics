-- Creator Analytics
-- Regression coverage for migration 031
-- File: Tests/Database/031_optimize_schedule_proposal_generation_regression.sql
--
-- This retains the complete migration-029 behavioral regression and adds
-- migration-031 index, helper-equivalence, view-contract, ACL, ordered-output,
-- and structural performance assertions. Every fixture is transactional and
...
-- ---------------------------------------------------------------------------
-- 0. Migration 031 optimization and compatibility guards
-- ---------------------------------------------------------------------------
...
begin
  if not exists (
    ...
    raise exception
      'Migration 031 proposal-history index is absent or changed';
  end if;
  ...
  if not exists (
    ...
    raise exception
      'Migration 031 scheduled-reservation index is absent or changed';
  end if;
  ...
  if not exists (
    ...
    raise exception
      'Migration 031 helper owner/security/volatility/ROWS/search_path changed';
  end if;
  ...
  if v_acl is distinct from array[
    'creator_dashboard_reader:EXECUTE',
    'service_role:EXECUTE'
  ]::text[] then
    raise exception
      'Migration 031 helper ACL changed: %',
      v_acl;
  end if;
  ...
  if regexp_count(
    ...
    raise exception
      'Hybrid view did not reduce two reservation scans to one helper call';
  end if;
  ...
  if regexp_count(
     ...
     or position('get_schedule_reservation_capacity' in v_definition) > 0 then
    raise exception
      'Proposal preview did not reduce three reservation scans to one helper call';
  end if;
  ...
  select
    ...
    count(*)::integer
  into v_contract_hash, v_contract_count
  ...
  if v_contract_count <> 93
     or v_contract_hash <>
        '957e19c9b255f2966a113850038e8e7204dd82652f50898395581ad85c1a3c9a' then
    raise exception
      'Hybrid view pre-031 column contract changed: count %, hash %',
      v_contract_count,
      v_contract_hash;
  end if;
  ...
  if v_contract_count <> 107
     or v_contract_hash <>
        '997d9be07e5dffbae1beaf1f45ed7945547470eec9de353b0b7b77d6cfa22e0d' then
    raise exception
      'Proposal preview pre-031 column contract changed: count %, hash %',
      v_contract_count,
      v_contract_hash;
  end if;
end;
...
-- Migration 031's unified helper must equal the two preserved migration-029
-- helpers for exact collisions, another channel, the open gap boundary, and
-- candidate self-exclusion.
do $test$
declare
  test_case record;
  unified_state record;
  legacy_conflict_count integer;
  legacy_daily_count integer;
  legacy_weekly_count integer;
begin
  for test_case in
    select *
    from (values
      (
        '__029_HISTORICAL_NEW_POST__'::text,
        '__029_INCIDENT_TIKTOK__'::text,
        '2026-08-24 20:00:00+00'::timestamptz
      ),
      (
        '__029_HISTORICAL_NEW_POST__'::text,
        '__029_TIKTOK_OTHER__'::text,
      ...
      (
        '__029_HISTORICAL_NEW_POST__'::text,
        '__029_INCIDENT_TIKTOK__'::text,
        '2026-08-25 02:00:00+00'::timestamptz
      ),
      (
        '__029_OLD_POST__'::text,
        '__029_INCIDENT_TIKTOK__'::text,
        '2026-08-24 20:00:00+00'::timestamptz
      )
    ) cases(candidate_post_id, buffer_channel_id, target_at_utc)
  loop
    select *
    into strict unified_state
    from public.calculate_schedule_reservation_state(
      test_case.candidate_post_id,
      test_case.buffer_channel_id,
      test_case.target_at_utc,
      6,
      'America/Denver',
      null
    );
    ...
    into strict legacy_daily_count, legacy_weekly_count
    ...
    if unified_state.reservation_conflict_count
         is distinct from legacy_conflict_count
       or unified_state.fixed_daily_reservation_count
         is distinct from legacy_daily_count
       or unified_state.fixed_weekly_reservation_count
         is distinct from legacy_weekly_count then
      raise exception
        'Unified/legacy reservation mismatch for % on % at %: unified %, legacy conflict/day/week %/%/%',
        test_case.candidate_post_id,
        test_case.buffer_channel_id,
        test_case.target_at_utc,
        unified_state,
  ...
  if unified_state.reservation_conflict_count is not null
     or unified_state.fixed_daily_reservation_count is not null
     or unified_state.fixed_weekly_reservation_count is not null then
    raise exception
      'Invalid minimum gap did not fail closed: %',
      unified_state;
  end if;
  ...
  from public.calculate_schedule_reservation_state(
    ...
    '2026-08-24 20:00:00+00',
    6,
    '__INVALID_TIMEZONE__',
    null
  );

  if unified_state.reservation_conflict_count is null
     or unified_state.fixed_daily_reservation_count is not null
     or unified_state.fixed_weekly_reservation_count is not null then
    raise exception
      'Invalid timezone did not preserve conflict diagnostics and fail capacity closed: %',
      unified_state;
  end if;
end;
$test$;
...
-- The hash was captured from the identical migrations-001-030 fixture. Dates
-- are normalized to the transaction anchor Monday, so this pins target choice
-- and ordering without depending on the wall-clock calendar date.
do $test$
declare
  v_ordered_target_hash text;
begin
  select encode(
    digest(
      coalesce(
        string_agg(
          format(
            ...
            proposal.proposed_due_at_local::date
              - test_clock.anchor_monday,
            proposal.proposed_due_at_local::time,
            proposal.slot_rank,
  ...
  if v_ordered_target_hash <>
     '47c8ab232e18b1626798263929a3ea668243fa04f65d38bdede32b29d1b9b9b5' then
    raise exception
      'Migration 031 changed pre-031 eight-post targets or ordering: %',
      v_ordered_target_hash;
  end if;
end;
$test$;
